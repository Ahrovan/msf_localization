...
{
    MatrixDense sum_result;

    // Check big sizes
    if(sum1.cols() != sum2.cols())
        throw;
    ...
    // Check blocks sizes
    //Check
    if(sum2.getColsSize() != sum1.getColsSize())
        throw;
    if(sum2.getRowsSize() != sum1.getRowsSize())
        throw;
    ...
    for(int i=0; i<sum2.rows(); i++)
        ...
        {
            // Two blocks non zero
            if( ( sum2(i,j).cols() != 0 && sum2(i,j).rows() != 0 )
